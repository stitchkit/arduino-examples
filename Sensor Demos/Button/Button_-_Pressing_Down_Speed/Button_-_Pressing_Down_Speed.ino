/* StitchKit.io Button Example - Running Lights Speed based on Holding and Pressing Button
 * -----------------------
 * This software uses the button sensor to turn speed up the effects in the Running lights LED example
 ...
 *  
 *  Scroll down and look for the "CHANGE CODE HERE" text to adjust the value or effect of the button press
 * 
 * Requires:
 * ---------
...
//Location of the button pin on the StitchKit board (it will always be A6)
const int buttonPin = A6;

// State of the button currently
int currentButtonState = 0;

// State of the button previously 
byte previousButtonState = LOW;

// How long the button was held (milliseconds)
long millis_held;  

// How long the button was held (seconds)
long secs_held;     

// How long the button was held previously (seconds)  
long prev_secs_held; 

// How long since the button was first pressed 
...
void RunningLights(byte red, byte green, byte blue, int WaveDelay) {
  ...
  for(int i=0; i<NUM_LEDS*2; i++)
  {
      ...
      //What is going on here is the sensor picks up whether the button is pressed, checks how long it is pressed and a value is adjusted
      ...
      if (millis_held > 50) {
        ...
        if (currentButtonState == HIGH && previousButtonState == HIGH) {
